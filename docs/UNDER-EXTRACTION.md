...
| Reverse video | off | `--reverse-video` | Looks inside solid blocks of ink for glyph-shaped holes — white lettering on black, which a filter looking for marks cannot see. Finds analyser readouts; also finds photographs, at about the same rate. Off until something separates the two. `docs/measurements/reverse-video.md`. |
